...
def train_network(filled_accumulators, training_normals):
    ...
    #save model
    model.save("dragon_model_noPCA_multi_1.h5")
...
def main():
    ...
    accumulator_filled = np.load("dragon_accumulator_noPCA_multi.npz")['accum'] 
    ...
    test_vertex_normals = readPointCloud("bunny.ply")
    #draw([test_vertex_normals])
    test_vertex_normals = test_vertex_normals.normals
    ...
    #Normalize test normals        
    for i in range(len(test_vertex_normals)):
        test_vertex_normals[i] = normalize(test_vertex_normals[i])
        if(np.dot(referencePoint, test_vertex_normals[i]) < 0):
            test_vertex_normals[i] *= -1 
    #Begin training
    print("Training network...")
    #model, test_x, test_y = train_network(accumulator_filled, training_vertex_normals)
    
    #load
    model = load_model("dragon_model_noPCA_multi_3.h5")
    
    #TEST DATA: model = load_model("accu_model_after_reorientation.h5")
    if (run_PCA):
        test_x = np.load("accumulator_filled_noPCA_multi3_Bunny.npz")['bunny_accumulators']
        test_y = np.delete(training_vertex_normals, 2, 1)
    else:    
        test_x = np.load("accumulator_filled_noPCA_multi3_Bunny.npz")['bunny_accumulators'] 
        test_y = test_vertex_normals 
    ...
    print(len(output_predictions_w_z))
        
    #put predicted
    Mesh_prediction = readPointCloud("bunny.ply") #copy.deepcopy(Mesh)
    ...
    #write
    writePointCloud("Output_Mesh_bunny_noPCA_multi_3.ply", Mesh_prediction)    
    
    #Angle adjustment
    angle_adjusts = []
    for this_normal in range(len(Mesh_prediction.normals)):
        #print(output_predictions_w_z[3484])
        normal_prediction = output_predictions_w_z[this_normal]
        #print(Mesh_copy.normals[3484])
        normal_true = Mesh_prediction.normals[this_normal]
        ...
        #convert to deg
        this_angle = np.rad2deg(this_angle)
        #Store in dataset
    ...
    print("")
    count10 = 0
    for i in range(len(angle_adjusts)):
        if (angle_adjusts[i] <= 10):
            count10 = count10 + 1
    ratio = count10/i
    print("Angles below 10")
    print(count10) #dragon = 118047 points
    print("Percentage")
    print(ratio)
